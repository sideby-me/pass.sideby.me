...
(function () {
  ...
  function scanMetaTags() {
    ...
    const ogVideoSecure = document.querySelector(
  ...
  function scanJsonScripts() {
    ...
    if (hostname.includes("tiktok.com")) {
      const rehydration = document.getElementById(
        ...
        try {
          ...
          if (itemStruct) {
            ...
            if (playAddr && playAddr.UrlList && playAddr.UrlList.length) {
              ...
              const urls = playAddr.UrlList.filter(
                (u) => !u.includes("v16-webapp-prime")
              );
              if (urls[0]) {
                sendVideo({
                  url: urls[0],
